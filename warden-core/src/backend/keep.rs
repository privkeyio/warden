...
mod implementation {
    ...
    impl<S: AuditStore + 'static> WardenSigningHooks<S> {
        ...
        fn spawn_post_sign_tasks(&self, session: &SessionInfo, signature: &[u8; 64]) {
            let ctx = match self.sessions.read().get(&session.session_id) {
                Some(ctx) => *ctx,
                None => return,
            };
    ...
    impl<S: AuditStore + 'static> SigningHooks for WardenSigningHooks<S> {
        fn pre_sign(&self, session: &SessionInfo) -> keep_frost_net::Result<()> {
            ...
            match sessions.get(&session.session_id) {
                Some(ctx) => match ctx.authorization {
                    SigningAuthorization::Approved => {
                        info!(
                    ...
                    SigningAuthorization::Pending => {
                        warn!(
                            workflow_id = %ctx.workflow_id,
                            "Pre-sign: workflow still pending approval"
                        );
                        Err(keep_frost_net::FrostNetError::PolicyViolation(
                            "Workflow approval pending".into(),
                        ))
                    }
                    SigningAuthorization::Rejected => {
                        warn!(
                            workflow_id = %ctx.workflow_id,
                            "Pre-sign: workflow was rejected"
                        );
                        Err(keep_frost_net::FrostNetError::PolicyViolation(
                            "Workflow rejected".into(),
                        ))
                    }
                },
                None => {
